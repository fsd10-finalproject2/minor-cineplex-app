...
const handleInput = (key: keyof MovieSearchParams, value: string | boolean | undefined) => {
...
<template>
  <div class="movie-filter flex flex-col gap-4 bg-gray-0 p-6 lg:p-5 rounded-xl border border-white/5">
    <!-- Main Search Row -->
    <div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:gap-3">
      <!-- Movie Search -->
    ...
    <!-- Accessibility & Clear Row -->
    <div class="flex flex-row justify-between items-center px-1">
      <div class="flex gap-6">
        <Checkbox 
          label="Wheelchair access" 
          :checked="modelValue.wheelchairAccess" 
          @update:checked="(v) => handleInput('wheelchairAccess', v)"
        />
        <Checkbox 
          label="Hearing assistance" 
          :checked="modelValue.hearingAssistance" 
          @update:checked="(v) => handleInput('hearingAssistance', v)"
        />
      </div>
      <button 
        @click="clearFilters" 
        class="text-white style-body-2-medium underline decoration-1 underline-offset-4 hover:text-blue-100 transition-colors"
      >
        Clear
      </button>
    </div>
