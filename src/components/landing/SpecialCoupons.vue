...
<template>
  <section class="flex flex-col items-center gap-10 py-10 xl:py-20 xl:px-30 w-full">
    <div class="flex flex-row justify-between items-end w-[90%] xl:w-[1200px]">
      ...
      <CustomButton
        variant="ghost"
        class="style-body-1-medium h-[44px] w-[51px] p-0!"
        @click="goToCouponsPage"
      >
        View all
      </CustomButton>
    </div>
    ...
    <div
      class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 w-fit xl:w-[1200px]"
      :aria-busy="isLoading"
      aria-live="polite"
    >
      <template v-if="isLoading">
        <CouponCardSkeleton
          v-for="n in SPECIAL_COUPONS_PAGE_SIZE"
